...
bool CDataFileReader::Open(class IStorage2 *pStorage, const char *pFilename, int StorageType)
{
...
bool CDataFileReader::GetCrcSize(class IStorage2 *pStorage, const char *pFilename, int StorageType, unsigned *pCrc, unsigned *pSize)
{
	IOHANDLE File = pStorage->OpenFile(pFilename, IOFLAG_READ, StorageType);
	if(!File)
		return false;
...
bool CDataFileWriter::Open(class IStorage2 *pStorage, const char *pFilename)
{
	dbg_assert(!m_File, "a file already exists");
	m_File = pStorage->OpenFile(pFilename, IOFLAG_WRITE, IStorage2::TYPE_SAVE);
	if(!m_File)
		return false;
