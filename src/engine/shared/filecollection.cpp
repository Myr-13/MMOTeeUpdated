...
void CFileCollection::Init(IStorage2 *pStorage, const char *pPath, const char *pFileDesc, const char *pFileExt, int MaxEntries)
{
	...
	m_pStorage->ListDirectory(IStorage2::TYPE_SAVE, m_aPath, FilelistCallback, this);
}
...
void CFileCollection::AddEntry(int64 Timestamp)
{
	...
	else
	{
		...
		if(m_NumTimestamps == m_MaxEntries)
		{
			...
			m_pStorage->RemoveFile(aBuf, IStorage2::TYPE_SAVE);
		}
